...
@section('content')
    <div class="mb-8 rounded border border-slate-800/70 bg-slate-900/40 p-6">
        ...
        <h1 class="mt-2 text-3xl font-semibold text-white">{{ $community->getTranslation('name', app()->getLocale()) }}</h1>
        <p class="mt-3 text-slate-400">{{ $community->getTranslation('description', app()->getLocale()) }}</p>
    </div>
    ...
    <div class="grid gap-8 lg:grid-cols-3">
            ...
            <div>
                ...
                <div class="space-y-3">
                    @forelse ($categories as $category)
                        <a href="{{ route('categories.show', $category) }}" class="block rounded border border-slate-800/70 bg-slate-900/40 p-4 transition hover:border-emerald-500/60 hover:bg-slate-900/70">
        ...
        <aside>
            <div class="rounded border border-slate-800/70 bg-slate-900/40 p-4">
                <div class="text-sm uppercase tracking-widest text-emerald-400">Community</div>
                <div class="mt-2 text-base font-semibold text-white">{{ $community->getTranslation('name', app()->getLocale()) }}</div>
