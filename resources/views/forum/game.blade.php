...
@section('content')
    <div class="mb-8 rounded border border-slate-800/70 bg-slate-900/40 p-6">
        <h1 class="text-3xl font-semibold text-white">{{ $game->getTranslation('name', app()->getLocale()) }}</h1>
        <p class="mt-3 text-slate-400">{{ $game->getTranslation('description', app()->getLocale()) }}</p>
    </div>
        ...
        <aside>
            ...
            <div class="space-y-3">
                @forelse ($communities as $community)
                    <a href="{{ route('communities.show', $community) }}" class="block rounded border border-slate-800/70 bg-slate-900/40 p-3 transition hover:border-emerald-500/60 hover:bg-slate-900/70">
